...
const ManagerSellerList = () => {
	...
	const columns = [
		...
		},
		{
			...
			key: "status",
			render: (_, record) => {
				return <span className={`${record.status === "Verified" ? "text-blue-500" : record.status === "Suspend" ? "text-red-500" : ""}`}>{record.status}</span>;
			},
		},
	];
	const data = [
				...
				<div className=" ml-[247px]">
					...
					>
						...
						>
							<p className="italic text-center -mt-3">
								<span className="font-medium">Id:</span> {sellerData.key}
							</p>
							...
							<Row justify={"space-between"} className="my-3">
								<label className="font-semibold" htmlFor="cooperationHistory">
									Cooperation History
								</label>
								<TextArea readOnly={true} value={sellerData.cooperationHistory} id="cooperationHistory" className="rounded-none" />
							</Row>
							<p className={`${sellerData.status === "Verified" ? "text-blue-500" : sellerData.status === "Suspend" ? "text-red-500" : ""}`}>
					...
					>
						<p className="text-base">
							Do you want to{" "}
							<span className={`${confirmStatus === "Verified" || confirmStatus === "Unsuspend" ? "text-blue-500" : "text-red-500"} font-semibold`}>
								{confirmStatus === "Verified" ? "verify" : confirmStatus.toLowerCase()}
							</span>{" "}
							this account?
						</p>
					</Modal>
